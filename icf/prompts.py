...
def build_extraction_prompt(var: TemplateVariable, protocol_length: int = 0) -> str:
    ...
    prompt += (
        ...
        "3. Once you have enough information, build the result and finish in ONE repl block:\n"
        "   ```repl\n"
        "   import json\n"
        "   result_dict = {\"section_id\": ..., \"status\": ..., ...}\n"
        "   result_json = json.dumps(result_dict)   # MUST be a string, not a dict\n"
        "   FINAL_VAR(result_json)                  # MUST be the last line of this block\n"
        "   ```\n"
        "   Do NOT put FINAL_VAR outside a code block. Do NOT pass a dict to FINAL_VAR.\n"
        "   If result_dict already exists from a previous iteration, skip straight to:\n"
        "   ```repl\n"
        "   FINAL_VAR(json.dumps(result_dict))\n"
        "   ```\n\n"
