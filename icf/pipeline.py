...
class ICFPipeline:
    ...
    def run(self) -> PipelineResult:
        ...
        if not self.skip_review and not self.skip_remediation and review_result:
            ...
            # Abbreviation / stubborn-term gloss consistency is checked
            # deterministically (see icf/abbreviations.py) and must not depend
            # on review having flagged something else first.
            from icf.abbreviations import find_abbreviation_fixes, find_term_gloss_fixes

            has_abbreviation_issues = bool(find_abbreviation_fixes(extractions, final_variables))
            has_gloss_issues = bool(find_term_gloss_fixes(extractions, final_variables))
            if remediable_count > 0 or has_notes or has_abbreviation_issues or has_gloss_issues:
                print(
                    ...
                    f"term-gloss fixes: {has_gloss_issues}) ..."
                )
